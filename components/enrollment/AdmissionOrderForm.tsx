...
function RemarksField({ value, onChange }: { value: string; onChange: (val: string) => void }) {
  ...
  return (
    <div>
      <label className="block text-lg font-medium text-gray-700 mb-1">Remarks</label>
      <textarea
        value={localValue}
    ...
    <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="relative group">
              {displayPhoto ? (
                ...
                <div className="w-20 h-24 rounded-lg border-2 border-dashed border-white/30 bg-white/5 flex flex-col items-center justify-center text-white/40 text-[12px] uppercase font-bold text-center px-1">
                  <span>Photo</span>
                  <span>Missing</span>
                </div>
              )}
            ...
            <div>
              <h1 className="text-5xl font-extrabold text-white tracking-tight">The American Foundation School</h1>
          ...
          <div className="text-right space-y-1">
            <div className="bg-white/10 px-3 py-1 rounded-full backdrop-blur-sm border border-white/20">
              <span className="text-base font-semibold text-red-500 bg-white px-2 py-0.5 rounded-full mr-2 uppercase">CC</span>
              <span className="font-mono text-2xl font-bold">{student.cc}</span>
            </div>
            <p className="text-base text-red-200 font-medium tracking-wide uppercase">GR: {student.gr_number || 'PENDING'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-8 space-y-8">
        ...
        <div className="grid grid-cols-4 gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100 items-center">
            ...
            <div className="flex flex-col">
                <span className="text-[12px] text-gray-500 uppercase font-bold tracking-wider">Academic Year</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
                <h3 className="text-lg font-black text-gray-400 uppercase tracking-widest border-b pb-2">Student Information</h3>
                <div className="grid gap-4">
                    <div className="group">
                        <label className="text-[12px] font-bold text-gray-500 uppercase mb-1 block group-focus-within:text-red-500 transition-colors">Candidate Name</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-[12px] font-bold text-gray-500 uppercase mb-1 block">Date of Birth</label>
                        ...
                        <div>
                            <label className="text-[12px] font-bold text-gray-500 uppercase mb-1 block">Reg Number</label>
                    ...
                    <div>
                        <label className="text-[12px] font-bold text-gray-500 uppercase mb-1 block">Permanent Address</label>
            ...
            <div className="space-y-6">
                <h3 className="text-lg font-black text-gray-400 uppercase tracking-widest border-b pb-2">Guardian Details</h3>
                <div className="grid gap-4">
                    <div className="group">
                        <label className="text-[12px] font-bold text-gray-500 uppercase mb-1 block">Father's Name</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-[12px] font-bold text-gray-500 uppercase mb-1 block">Father Cell</label>
                        ...
                        <div>
                            <label className="text-[12px] font-bold text-gray-500 uppercase mb-1 block">Mother Cell</label>
                    ...
                    <div className="p-4 bg-orange-50 border border-orange-100 rounded-xl space-y-1">
                        <span className="text-[12px] font-black text-orange-600 uppercase tracking-wider">Emergency Contact</span>
                        <p className="font-bold text-orange-900">{student.nearest_name || 'N/A'}</p>
                        <p className="text-lg text-orange-800 font-medium">{student.nearest_phone || 'No Phone Registered'}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-8 border-t border-gray-200">
          <div className="flex items-center gap-3">
            {isPreparingAssets ? (
              <div className="flex items-center gap-3 px-4 py-2 bg-yellow-50 text-yellow-700 rounded-full border border-yellow-200 text-lg font-bold shadow-sm">
                <Loader2 className="h-4 w-4 animate-spin" />
                Validating Print Assets...
              </div>
            ) : photoBase64 ? (
              <div className="flex items-center gap-3 px-4 py-2 bg-green-50 text-green-700 rounded-full border border-green-200 text-lg font-bold shadow-sm animate-in fade-in slide-in-from-left-4">
                <div className="h-2 w-2 bg-green-500 rounded-full" />
                Asset Ready for High-Res Print
              </div>
            ) : photoError ? (
              <div className="flex items-center gap-2 px-4 py-2 bg-orange-50 text-orange-700 rounded-full border border-orange-200 text-lg font-bold shadow-sm">
                <AlertTriangle className="h-4 w-4" />
                Photo unavailable — PDF will print without photo
              </div>
            ) : student.photograph_url ? (
                <div className="flex items-center gap-3 px-4 py-2 bg-blue-50 text-blue-700 rounded-full border border-blue-200 text-lg font-bold shadow-sm">
                    Photo Loading...
                </div>
            ) : (
                <div className="flex items-center gap-3 px-4 py-2 bg-gray-100 text-gray-600 rounded-full border border-gray-200 text-lg font-bold shadow-sm">
