...
import {
  Download, FileText, Info, LayoutDashboard, Loader2,
} from "lucide-react";
  ...
  useEffect(() => {
    ...
    const fetchRows = async () => {
      ...
      try {
        ...
        if (cancelled) return;
        setItems(view === "heads" ? (data?.data?.items ?? []) : []);
        setStudentItems(view === "student" ? (data?.data?.items ?? []) : []);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex gap-1 p-1 rounded-xl bg-zinc-100 dark:bg-zinc-900">
            <button
              type="button"
      ...
      <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-[24px] p-5 space-y-2">
        ...
        <p className="text-[11px] text-zinc-400 font-medium ml-1">
          Not issued heads are scheduled but not yet billed. Leaving them in inflates expected relative to what has gone out on a voucher.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-[24px] shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-zinc-100 dark:border-zinc-800">
          <span className="text-sm font-bold text-zinc-700 dark:text-zinc-300">
            {isLoading ? "Loading…" : view === "student"
              ? `${(pagination?.total ?? 0).toLocaleString()} students`
              : `${(pagination?.total ?? 0).toLocaleString()} fee heads`}
          </span>
        </div>
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest">
              {view === "student" ? "Fetching students…" : "Fetching fee heads…"}
            </p>
          </div>
        ) : (view === "student" ? studentItems.length === 0 : items.length === 0) ? (
          <div className="py-24 text-center text-sm text-zinc-400">
            {view === "student" ? "No students with fee heads in this range." : "No fee heads in this range."}
          </div>
        ) : view === "student" ? (
          <div className="overflow-auto">
            <table className="w-full border-collapse text-sm">
              <thead className="sticky top-0 z-10 bg-zinc-50 dark:bg-zinc-900/95 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800">
                <tr>
                  {["CC", "GR", "Name", "Campus", "Class", "Section", "Heads", "Amount", "Paid", "Outstanding"].map((h) => (
                    <th
                      key={h}
                      className={`px-4 py-3 text-[10px] font-black uppercase tracking-widest text-zinc-400 ${["Heads", "Amount", "Paid", "Outstanding"].includes(h) ? "text-right" : "text-left"}`}
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {studentItems.map((row) => (
                  <tr key={row.cc} className="border-t border-zinc-100 dark:border-zinc-800">
                    ...
                    <td className="px-4 py-3 text-right tabular-nums text-zinc-500">{row.head_count.toLocaleString()}</td>
