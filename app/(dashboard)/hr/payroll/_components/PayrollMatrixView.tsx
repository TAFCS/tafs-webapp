...
const LEAVE_SHORT_LABEL: Partial<Record<DayClassification, string>> = {
  SICK_LEAVE:   "Sick",
  CASUAL_LEAVE: "Casual",
  ANNUAL_LEAVE: "Annual",
  UNPAID_LEAVE: "Unpaid",
...
/**
 * Memoized so typing in the parent's search box (or any other parent state
 * change) doesn't re-render every cell of every row. `dates` and `onOpenLine`
 * are kept referentially stable by the parent for this to bite.
 */
const MatrixRow = memo(function MatrixRow({ line, dates, onOpenLine }: RowProps) {
  ...
  const dayMap = useMemo(
    () => new Map<string, DayBreakdownEntry>(line.daily_breakdown.map((d) => [d.date, d])),
  ...
  return (
    <tr className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/20">
      ...
      {dates.map((d) => {
        ...
        // A holiday/weekend where the employee still punched in — surface it
        // instead of hiding it, so HR notices and can override the day.
        const cameOnOff = isOff && punches.length > 0;
        const leaveLabel = LEAVE_SHORT_LABEL[cls];
...
export function PayrollMatrixView({ periodStart, periodEnd, lines, onOpenLine }: Props) {
  ...
  // filtering upstream (search, campus, department) implicitly falls back to
  // page 1 without an effect. Keyed on primitives, not the `lines` array —
  // one caller rebuilds that array on every render.
  ...
  // rows one identity that never changes — otherwise every row re-renders on
  // every parent keystroke and the memo buys nothing.
  const openLineRef = useRef(onOpenLine);
  useEffect(() => { openLineRef.current = onOpenLine; }, [onOpenLine]);
  const handleOpenLine = useCallback(
    (line: AttendanceLineBase, date: string) => openLineRef.current(line, date),
    [],
  );
        ...
        <div className="overflow-auto max-h-[calc(100vh-220px)]">
          <table className="border-collapse text-xs" style={{ minWidth: `${200 + dates.length * 88}px` }}>
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {pageLines.map((line) => (
                <MatrixRow key={line.employee_id} line={line} dates={dates} onOpenLine={handleOpenLine} />
              ))}
            </tbody>
          </table>
        </div>
