...
import { attendanceService, StaffTimeline, TimelineSegmentType } from "@/lib/attendance.service";
...
const SEGMENT_STYLES: Record<TimelineSegmentType, { bg: string; label: string }> = {
    ...
    OVERTIME: { bg: "bg-orange-500", label: "Over time" },
    DAY_OFF: { bg: "bg-amber-400", label: "Requested day off" },
};
...
export default function StaffAttendanceTimelinePage() {
    ...
    const load = useCallback(async () => {
        ...
        setError(null);
        try {
            const data = await attendanceService.getStaffTimeline(employeeId, { date_from: dateFrom, date_to: dateTo });
            setTimeline(data);
        } catch {
