...
function AdmissionRow({ admission, studentCc, onSaved, onDeleted }: { admission: any; studentCc: number; onSaved: (a: any) => void; onDeleted: () => void }) {
    ...
    return (
        <div className={`bg-white border rounded-2xl p-4 space-y-3 transition-all ${isDirty ? "border-amber-200 shadow-sm" : "border-zinc-100"}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
                <Field label="Academic System"><Input value={local.academic_system ?? ""} onChange={v => set("academic_system", v)} placeholder="e.g. Cambridge" /></Field>
                <Field label="Grade"><Input value={local.requested_grade ?? ""} onChange={v => set("requested_grade", v)} placeholder="e.g. Grade 6" /></Field>
                <Field label="Academic Year"><Input value={local.academic_year ?? ""} onChange={v => set("academic_year", v)} placeholder="e.g. 2024-25" /></Field>
...
export function AdmissionsTab({ student, onReload }: { student: any; onReload: () => void }) {
        ...
        <div className="space-y-3">
            ...
            {newRow && (
                <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 space-y-3 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <Field label="Academic System"><Input value={newRow.academic_system} onChange={v => set("academic_system", v)} /></Field>
                        <Field label="Grade"><Input value={newRow.requested_grade} onChange={v => set("requested_grade", v)} /></Field>
                        <Field label="Academic Year"><Input value={newRow.academic_year} onChange={v => set("academic_year", v)} /></Field>
