...
export function TransferOrderTab({ cc }: Props) {
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center p-20 bg-zinc-50 rounded-2xl border border-dashed border-zinc-200">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Loading Transfer Data...</p>
            </div>
        );
    }
    ...
    if (!studentData) return (
        <div className="p-10 text-center">
            ...
            <p className="text-zinc-500 font-bold">Could not load transfer order.</p>
        </div>
    );

    return (
        <div className="bg-zinc-50 -m-6 p-6 min-h-screen">
            <TransferOrderForm student={studentData} alreadyTransferred={true} />
        </div>
    );
}
